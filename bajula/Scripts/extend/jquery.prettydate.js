...
(function () {
    ...
    $.prettyDate = {
        ...
        now: function () {
            return Date.now().addMinutes(tradelr.date.getTimezoneOffset());
        },
        ...
        format: function (time) {
            ...
            if (isNaN(day_diff) || day_diff < 0) {
                if (day_diff < 0) {
                    // set to 'just now' to cater for variations
                    day_diff = 0;
                    diff = 0;
                }
                else {
			...
			day_diff < 30 && messages.weeks(Math.floor(day_diff / 7)) ||
            day_diff < 60 && messages.month ||
            day_diff >= 60 && messages.months(Math.floor(day_diff / 30));
        }

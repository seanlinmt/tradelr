...
namespace DotLiquid
{
	public class Context
	{
        ...
        // used to store instance specific values
        public Hash Values { get; set; }

		public Context(List<Hash> environments, Hash outerScope, Hash registers, Hash values, bool rethrowErrors)
		{
		...
		public Context()
            : this(new List<Hash>(), new Hash(), new Hash(), new Hash(), false)
		{
        ...
        public object GetScoped(string key, string scope)
        {
            return Resolve(key, scope);
        }

        public void SaveScoped(string key, object value, string scope)
        {
            var scopedkey = string.Concat(key,"_",scope).Replace(".", "_");
            Scopes[0][scopedkey] = value;
        }

		public bool HasKey(string key, string scope = "")
		{
			return Resolve(key, scope) != null;
		}
		...
		/// <param name="key"></param>
		/// <returns></returns>
		private object Resolve(string key, string scope = "")
		{
		    if (!string.IsNullOrEmpty(scope))
		    {
		        key = string.Concat(key, "_", scope).Replace(".", "_");
		    }
			switch (key)
			{
				case null:

...
<div id="order_container">
        ...
        <% } %>
           <div class="order_address_sender" title="Sender name & address">
                ...
                <h2>invoice <strong>#<%= Model.order.orderNumber.ToString("D8")%></strong></h2>
                </div>
        ...
        <div id="order_side">
            ...
            <% if (!string.IsNullOrEmpty(Model.order.shippingAddress))
              ...
              <% } %>
                <% if (!string.IsNullOrEmpty(Model.order.billingAddress))
               {%>
                <h4>
                    Billing Address</h4>
            ...
            <% if (!string.IsNullOrEmpty(Model.order.paymentMethod))
               { %>
                <h4 class="m0">
                    Payment Method</h4>
                <div>
                    <strong>Payment by <%=Model.order.paymentMethod%></strong>. <%=Model.order.paymentInstructions%></div>
                    <% } %>
                    <% if (!string.IsNullOrEmpty(Model.order.shippingNotes))
                       { %>
                <h4>
                    Shipping Method</h4>
                <div>
                    <%= Model.order.shippingNotes %></div>
                    <% } %>
                <h4>
                    Terms</h4>
                <div>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="orderitem_unitPrice">
                        <%= item.quantity %> x <%=item.UnitPrice.ToString("n" + Model.currency.decimalCount) %>
                    </td>
